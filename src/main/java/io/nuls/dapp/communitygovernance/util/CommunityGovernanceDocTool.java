...
import io.nuls.dapp.communitygovernance.api.CommunityGovernanceApi;
...
public class CommunityGovernanceDocTool {
    ...
    public static class Gen {
        ...
        public static List<CmdDes>[] buildData() {
            ...
            Class<?> clazs = CommunityGovernanceApi.class;
            ApiType apiType = ApiType.RESTFUL;
            String cmdBaseName = "";
            String methodPath = "";
            boolean restful = true;
            if (restful) {
                RequestMapping annotation = clazs.getAnnotation(RequestMapping.class);
                cmdBaseName = annotation.value()[0];
            }
        ...
        private static void writeMarkdown(CmdDes cmd, BufferedWriter writer, AtomicInteger i, Map<String, Object> egMap) {
            try {
                ...
                if (i != null) {
                    ...
                    if (cmd.order < 100) {
                        order = repeat("0", 3 - order.length()) + order;
                    }
